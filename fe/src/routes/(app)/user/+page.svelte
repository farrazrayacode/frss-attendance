      ...
      <div class="flex flex-col gap-y-4 px-6 py-5 lg:gap-y-6">
              ...
              <table class="min-w-full">
                  ...
                  <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                      {#if $usersQuery.isLoading}
                          <tr><td colspan="9" class="p-4 text-center">Loading users...</td></tr>
                      {:else if $usersQuery.isError}
                          <tr><td colspan="9" class="p-4 text-center">Error loading users. Please check backend.</td></tr>
                      {:else if !$usersQuery.data || $usersQuery.data.length === 0}
                          <tr><td colspan="9" class="p-4 text-center">No users found.</td></tr>
                      {:else}
                              ...
                              <tr>
                                  ...
                                  <td class="px-5 py-4 sm:px-6">
                                      <div class="flex items-center gap-x-1">
                                          ...
                                          <span class="text-theme-sm text-gray-500 dark:text-gray-400">{user.isOnline ? 'Online' : 'Offline'}</span>
                                      </div>
                                  </td>
                                  <td class="px-5 py-4 sm:px-6">
                                  ...
                                  <td class="px-5 py-4 sm:px-6">
                                      <div class="flex items-center gap-x-2">
                                          {#if user.isApproved}
                                              <button aria-label="editButton" class="btn-secondary-icon"><PencilLine class="h-4 w-4" /></button>
                                              <button aria-label="resetPasswordButton" class="btn-secondary-icon"><Key class="h-4 w-4" /></button>
                                              <button
          ...
          <div class="flex flex-col items-center gap-y-4 lg:flex-row lg:justify-between">
              ...
              <div class="flex items-center gap-x-3">
                  <button aria-label="previousButton" class="btn-secondary-icon"><ChevronLeft class="h-5 w-5" /></button>
                  <div class="flex items-center gap-x-1">
                  ...
                  <button aria-label="nextButton" class="btn-secondary-icon"><ChevronRight class="h-5 w-5" /></button>
              </div>
              <div class="flex items-center gap-x-2">
                  ...
                  <div class="relative z-20 bg-transparent">
                      <select class="select-input">
                          {#each [10, 20, 30, 40, 50] as perRows}
                              <option value={perRows} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">{perRows}</option>
                          {/each}
                      </select>
                      <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                          <ChevronDown class="h-5 w-5" />
              ...
              <table class="min-w-full">
                  ...
                  <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                      {#if $rolesQuery.isLoading}
                          <tr><td colspan="5" class="p-4 text-center">Loading roles...</td></tr>
                      {:else if $rolesQuery.isError}
                          <tr><td colspan="5" class="p-4 text-center">Error loading roles.</td></tr>
                      {:else if !$rolesQuery.data || $rolesQuery.data.length === 0}
                          <tr><td colspan="5" class="p-4 text-center">No roles found.</td></tr>
                      {:else}
                          {#each $rolesQuery.data as role, index}
                              <tr>
                                  ...
                                  <td class="px-5 py-4 sm:px-6"><p class="text-theme-sm text-gray-500 dark:text-gray-400">{(role as any).usersCount || 0}</p></td>
                                  <td class="px-5 py-4 sm:px-6"><p class="text-theme-sm text-gray-500 dark:text-gray-400">{role.permissions?.join(', ') || 'None'}</p></td>
                                  <td class="px-5 py-4 sm:px-6">
                                      <div class="flex items-center gap-x-2">
                                          <button aria-label="editRole" class="btn-secondary-icon"><PencilLine class="h-4 w-4" /></button>
                                          <button aria-label="deleteRole" class="btn-secondary-icon"><Trash class="h-4 w-4" /></button>
                                      </div>
                                  </td>
                              </tr>
                          {/each}
